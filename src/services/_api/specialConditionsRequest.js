...
export const getAllSpecialConditions = async () => {
  ...
  const response = await fetch(API_BASE_URL + '/special_conditions', requestOptions);
  return await response.json();
};

export const postCustomerSpecialConditions = async (data) => {
  const myHeaders = new Headers();
  myHeaders.append('Content-Type', 'application/json');
  const raw = JSON.stringify(data);
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/customer_special_conditions`, requestOptions);
