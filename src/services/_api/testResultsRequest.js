...
export const getAllTestResults = async () => {
  ...
  const response = await fetch(API_BASE_URL + '/test-results', requestOptions);
...
export const getTestResultsById = async (id) => {
  ...
  const response = await fetch(API_BASE_URL + '/test-results/' + id, requestOptions);
  return await response.json();
};
...
export const putTestResults = async (id, data) => {
  const myHeaders = new Headers();
  myHeaders.append('Content-Type', 'application/json');
  const raw = JSON.stringify(data);
