import { require_react_dom } from './chunk-ZIYUGMGG.js';
import { require_react } from './chunk-65KY755N.js';
...
function createRouter(init) {
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    let matches =
      opts != null && opts.initialHydration && state.matches && state.matches.length > 0 && !initialMatchesIsFOW
  ...
  async function handleLoaders(
    ...
    cancelActiveDeferreds(
      (routeId) =>
        !(matches && matches.some((m) => m.route.id === routeId)) || (matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId))
    );
  ...
  async function startRedirectNavigation(request, redirect3, isNavigation, _temp2) {
    let { submission, fetcherSubmission, preventScrollReset, replace: replace2 } = _temp2 === void 0 ? {} : _temp2;
  ...
  async function callLoadersAndMaybeResolveData(state2, matches, matchesToLoad, fetchersToLoad, request) {
    ...
    await Promise.all([
      resolveNavigationDeferredResults(matches, loaderResults, request.signal, currentMatches, state2.loaderData),
  ...
  async function discoverRoutes(matches, pathname, signal) {
    ...
    while (true) {
      ...
      if (
        !newPartialMatches ||
        (partialMatches.length === newPartialMatches.length && partialMatches.every((m, i) => m.route.id === newPartialMatches[i].route.id))
      ) {
        return {
          type: 'success',
          matches: null
...
function isNewRouteInstance(currentMatch, match) {
  ...
  return (
    ...
    // e.g. /files/images/user/avatar.jpg -> files/finances.xls
...
function patchRoutesImpl(routeId, children, routesToUse, manifest, mapRouteProperties2) {
  ...
  let newRoutes = convertRoutesToDataRoutes(
    uniqueChildren,
    mapRouteProperties2,
    [routeId || '_', 'patch', String(((_childrenToPatch = childrenToPatch) == null ? void 0 : _childrenToPatch.length) || '0')],
    manifest
  );
...
` +
  ...
  if (locationArg) {
    ...
    !(
      parentPathnameBase === '/' ||
      ((_parsedLocationArg$pa = parsedLocationArg.pathname) == null ? void 0 : _parsedLocationArg$pa.startsWith(parentPathnameBase))
    )
      ? true
        ? invariant(
            false,
  ...
  let matches =
    !isStatic && dataRouterState && dataRouterState.matches && dataRouterState.matches.length > 0
    ...
    for (let i = 0; i < renderedMatches.length; i++) {
      ...
      if (match.route.id) {
        ...
        let needsToRunLoader =
          match.route.loader && loaderData[match.route.id] === void 0 && (!errors2 || errors2[match.route.id] === void 0);
...
function Router(_ref5) {
  ...
  !!useInRouterContext()
    ? true
      ? invariant(false, 'You cannot render a <Router> inside another <Router>. You should never have more than one in your app.')
      : invariant(false)
    : void 0;
  ...
  let setState = React2.useCallback(
    (newState, _ref2) => {
      ...
      let isViewTransitionUnavailable =
        router.window == null || router.window.document == null || typeof router.window.document.startViewTransition !== 'function';
...
var Link = React2.forwardRef(function LinkWithRef(_ref7, ref) {
  let { onClick, relative, reloadDocument, replace: replace2, state, target, to, preventScrollReset, viewTransition } = _ref7,
...
var NavLink = React2.forwardRef(function NavLinkWithRef(_ref8, ref) {
  ...
  let nextLocationPathname =
    routerState && routerState.navigation && routerState.navigation.location ? routerState.navigation.location.pathname : null;
  if (!caseSensitive) {
  ...
  if (typeof classNameProp === 'function') {
    ...
    className = [classNameProp, isActive ? 'active' : null, isPending ? 'pending' : null, isTransitioning ? 'transitioning' : null]
      .filter(Boolean)
      .join(' ');
  }
  ...
  if (typeof document !== 'undefined') {
    ...
    React2.useLayoutEffect(() => {
      ...
      let disableScrollRestoration =
        router == null ? void 0 : router.enableScrollRestoration(savedScrollPositions, () => window.scrollY, getKeyWithoutBasename);
